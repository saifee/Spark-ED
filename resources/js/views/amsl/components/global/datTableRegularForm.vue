    ...
    <div class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
          ...
          <div class="response_flex">
            ...
            >
              <button
                ...
                @click.prevent="assainSearchValue(),searched=true"
              >
                Search
              </button>
            </div>
            ...
            <div class="printableArea">
              <a
                ...
                @click.prevent="$parent.callModal(true)"
              >
      ...
      >
        ...
        >
          ...
          <table
            id="table1"
            class="table table-striped  table-fw-widget dataTable no-footer table_block regularTable"
            role="grid"
          ...
          >
            <thead>
              <tr>
                ...
                <th
                  colspan="4"
                  style="background: #B2B1A8;"
                >
                  <strong>Room Sales</strong>
                </th>
                <th style="background:#FA1100; color: #fff">
                  Parking
                </th>
                <th style="background:#FA1100" />
                <th
                  colspan=""
                  style="background:#FA1100; color: #fff"
                >
                  Internal A/C
                </th>
                <th class="" />
                <th class="regular-head-print" />
                <th class="regular-head-print" />
                <th class="regular-head-print" />
              </tr>
              <tr role="row tableHeader">
                <th
              ...
              <tr>
                ...
                >
                  <div class="totalAmount">
                    Total (RM Total - Return ) =  <strong> {{ (rmTotalAmount-returnTotalAmount).toFixed(2) }} </strong>
                  </div>
                </td>
                <td />
              </tr>
                ...
                if (!_.isEmpty(this.ajax)) {
                    axios.get(this.ajax.url, {
                        ...
                        if (this.paging) {
                            ...
                            var totalReturn=0
                            var totalEvo=0
                            var totalParkCashAmount=0
                            var totalParkCardAmount=0
                            var totalOtherSales=0
                            var totalAdvanceSales=0
                            this.dataList.forEach(value=>{
                                var rmTotal=0
                                var total=0
                                if(value.cash||value.bank|| value.eviivo){
                                    var cash=Number(value.cash)
                                    var bank=Number(value.bank)
                                    var eviivoAmount=Number(value.eviivo)
                                    rmTotal = rmTotal + cash+ bank+eviivoAmount
                                    value['rm_total']=rmTotal.toFixed(2)
                                    totalCash=totalCash+cash
                                    totalBank=totalBank+bank
                                    totalEvo=totalEvo+eviivoAmount
                                }

                                if(value.parking_cash|| value.parking_card || value.return || value.other_sales || value.advance_sales){

                                    var parkingCash=Number(value.parking_cash)
                                    var parkingCard=Number(value.parking_card)
                                    var otherSales=Number(value.other_sales)
                                    var advanceSales=Number(value.advance_sales)
                                    total = total +parkingCash+ parkingCard
                                    totalReturn = totalReturn+ Number(value.return)
                                    value['total']=total.toFixed(2)
                                    totalParkCashAmount=totalParkCashAmount+parkingCash
                                    totalParkCardAmount=totalParkCardAmount+parkingCard
                                    totalOtherSales=totalOtherSales+otherSales
                                    totalAdvanceSales=totalAdvanceSales+advanceSales
                            ...
                            this.dataList[this.dataList.length]={
                                ...
                                eviivo: totalEvo.toFixed(2),
                                id:'balance' ,
                                mr: "",
                                name: "Balance",
                                parking_card: totalParkCardAmount.toFixed(2),
                                parking_cash: totalParkCashAmount.toFixed(2),
                                ref: '',
                                return: totalReturn.toFixed(2),
                                other_sales: totalOtherSales.toFixed(2),
                                advance_sales: totalAdvanceSales.toFixed(2),
                                remark: '',
                                rm_total: (totalCash+totalBank+totalEvo).toFixed(2),
                                total: (totalParkCashAmount+totalParkCardAmount).toFixed(2),
                            }
                            this.returnTotalAmount=totalReturn
                            this.rmTotalAmount=totalEvo+totalBank+totalCash
