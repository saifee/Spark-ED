                ...
                <div class="container-fluid">
                    ...
                    <div class="row mt-3" v-if="meeting.is_live">
                        ...
                        <div class="col" v-if="is_joined && can_share_screen">
                            ...
                            <button disabled class="btn btn-block btn-info" v-if="is_screen_shared" @click="stopShare">{{trans('communication.stop_share_screen')}}</button>
                        </div>
                        <div class="col" v-if="is_joined">
                        ...
                        <template v-if="meeting.is_live">
                            ...
                            <template v-if="is_joined">
                                <div v-for="(item, index) in orderedVideoList" :key="item.id" :class="['video-item', index === 0 || screen_width <= 768 ? 'video-maximized' : 'video-minimized', item.maximized ? 'maxmized' : '' ]">
                                    ...
                                    <p class="mt-3">{{item.fullName}} <span v-if="item.maximized != -1" class="ml-2 custom-button" @click="highlight(item)"><i class="fas fa-expand-arrows-alt"></i></span> <span v-if="index === 0" class="ml-2 custom-button" @click="fullScreen(item)"><i class="fas fa-expand"></i></span> </p>
                                </div>
                            </template>
                        </template>
                    </div>
...
export default {
    ...
    mounted() {
        ...
        this.rtcmConnection.onstreamended = function(stream) {
            let newList = []
            vm.videoList.forEach(function(item) {
                if (item.id !== stream.streamid) {
                    newList.push(item)
                }
            })
            vm.videoList = newList
        }
