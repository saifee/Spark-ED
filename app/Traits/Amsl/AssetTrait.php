...
trait AssetTrait
{
    ...
    public function getCurrentAssetAr(){
        ...
        $data=DB::table('amsl_accounts as accounts')
            ->leftJoin('amsl_assets as assets', 'accounts.id', '=', 'assets.account_id')
            ->select('accounts.id as aid', 'accounts.name', DB::raw("(SELECT SUM(CASE  WHEN assets.transaction_type ='Receive' THEN -assets.amount ELSE assets.amount END) from amsl_assets as assets where assets.account_id=aid and asset_date between '$this->fromDate' and '$this->toDate' )as assetAmount"), DB::raw("(SELECT sum(amount) from amsl_incomes WHERE asset_id = aid and income_date between '$this->fromDate' and '$this->toDate')as incomeAmount"),
                DB::raw("(SELECT sum(amount) from amsl_liabilities WHERE asset_id = aid and liability_date between '$this->fromDate' and '$this->toDate')as liabilitiesAmount")
